                                                                                            ...
                                                                                            chrome.serial.getDevices(function(devices) {
                                                                                                ...
                                                                                                for (var i = 0; i < ports.length; i++) {
                                                                                                    if (ports[i] == selected_port) {
                                                                                                        ...
                                                                                                        chrome.serial.connect(selected_port, {bitrate: selected_baud}, onOpen);
                                                                                                    }
                                                                                                }
                                                                                            });
                                                                                        }, 50, true);
...
function onOpen(openInfo) {
    ...
    if (connectionId != -1) {
        ...
        chrome.serial.onReceive.addListener(serial_read);
        ...
        GUI.timeout_add('quick_join', function() { // quick_join timer triggered / expired, we will continue with standard connect procedure
            ...
            chrome.serial.onReceive.removeListener(serial_read); // standard connect sequence uses its own listener
            ...
            chrome.serial.setControlSignals(connectionId, {dtr: true}, function(result) {
                ...
                chrome.serial.onReceive.addListener(function startup_listener(info) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        if (data[i] != 13) { // CR
                            ...
                            if (startup_message_buffer == "OpenLRSng TX starting") {
                                ...
                                chrome.serial.onReceive.removeListener(startup_listener);
